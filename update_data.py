...
def calculate_risk_scores(
    ...
    log.info(
        "  Weighted: news=%.1f flight=%.1f tanker=%.1f weather=%.1f poly=%.1f pent=%.1f",
...
async def run_pipeline(weather_api_key: str) -> None:
    ...
    async with aiohttp.ClientSession() as session:
        # Fetch polymarket, news, aviation, weather in parallel
        ...
        polymarket_result, news_result, aviation_result, weather_result = await asyncio.gather(
            polymarket_task, news_task, aviation_task, weather_task,
        )
...
def main() -> None:
    ...
    async def _loop():
        while not shutdown_event.is_set():
            try:
                await run_pipeline(weather_api_key)
            except Exception:
